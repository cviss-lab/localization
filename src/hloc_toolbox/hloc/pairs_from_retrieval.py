...
def main(descriptors, output, num_matched,
         ...
         db_prefix=None, db_list=None, db_model=None, db_descriptors=None, images=None):
    ...
    for query, indices in zip(query_names, topk):
        for i in indices:
            ...
            if images is not None:
                shutil.copy(path.join(str(images), db_names[i]), path.join(str(output.parent), 'matches', db_names[i]))
