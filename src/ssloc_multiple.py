...
class Node:
    ...
    def __init__(self, ros=True, debug=False, data_folder=None, create_new_anchors=False):
        ...
        if self.ros:
            ...
            self.optimization = rospy.get_param('~panorama/optimization',default=True)        
            self.max_reproj_error = rospy.get_param('~panorama/max_reproj_error',default=15)        
        ...
        if self.ros:
            ...
            rospy.Subscriber('/panorama_image',CompressedImage, self.callback_query_panorama)
            self.panorama_pub = rospy.Publisher('/panorama_image2/compressed',CompressedImage, queue_size=1)
    ...
    def callback_query_panorama(self,*args):
        print('query panorama image recieved!')
        # try:
        msg = args[0]
        query_frame_id = msg.header.frame_id      
        timestamp_query = msg.header.stamp            

        cv_bridge = CvBridge()
        Ip = cv_bridge.compressed_imgmsg_to_cv2(msg)         
        ...
        for k in range(4):
            ...
            pairs,scores = search.main(fname2_global,fdir_db,num_matches=self.num_retrieved_anchors)
        ...
        T_m1_m2,tvecs_l,rvecs_l,best_inlier_idxs=utils.multiviewSolvePnPRansac(pts3D_all, pts2D_all, query_poses_l, K2, max_reproj_error=self.max_reproj_error) 
        ...
        # send localized pose relative to robot map
        msg = cv_bridge.cv2_to_compressed_imgmsg(Ip)
        T_m1_c2=self.send_reloc_pose(C,R,query_frame_id,msg.header.stamp)
        self.panorama_pub.publish(msg)

        print('\nquery panorama camera localized!\n')  

        print('\nInliers in View 1: %i' % len(best_inlier_idxs[0]))
        print('Inliers in View 2: %i' % len(best_inlier_idxs[1]))
        print('Inliers in View 3: %i' % len(best_inlier_idxs[2]))
        print('Inliers in View 4: %i' % len(best_inlier_idxs[3]))
